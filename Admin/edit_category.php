          ...
          <?php 
            $role = $user_obj->getRole();
             ...
             if (isset($_POST['u_add_cat'])) {
               $cat_t=$_POST['u_cat_title'];
               $cat_e = mysqli_query($connection,"SELECT * FROM category WHERE cat_title='$cat_t'");
              if(empty($cat_t)){
                 echo "<div class='container'><div class=' alert alert-block alert-danger fade in text-center col-lg-6'>Can't be update a empty category</div></div>";
             }elseif(mysqli_num_rows($cat_e) > 0){
                   echo "<div class='container'><div class=' alert alert-block alert-danger fade in text-center col-lg-6'>It's a existing category</div></div>";
              }else{
