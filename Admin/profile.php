...
if(isset($_GET['user_id'])) {
  ...
  $query = mysqli_query($connection, "SELECT * FROM users WHERE id=$user_id AND username='$user'");
  ...
  if(isset($_POST['update_data'])) {
    ...
    $res_u = mysqli_query($connection,"SELECT * FROM users WHERE username='$user_name'");
    if(mysqli_num_rows($res_u) > 0){
    header("Location: profile.php?user_id=$id&message=username_already_taken");
    }else{
    $sql = mysqli_query($connection, "UPDATE users SET username='$user_name',firstname='$f_name',lastname='$l_name',email='$u_email' WHERE id='$id'");
  ...
  if(isset($_POST['update_password'])) {
    ...
    $sql = mysqli_query($connection, "UPDATE users SET password='$newhashpwd' WHERE id='$id' AND username='$user'");
    if($sql){
       header("Location: profile.php?user_id=$id&message=password_updated");
    }
    }
  }
                    ...
                    <section class="panel">
                      ...
                      <div class="panel-body bio-graph-info">
                        ...
                        <form class="form-horizontal" role="form" method="POST" action="profile.php?user_id='<?php echo $id; ?>'">
                          ...
                          <div class="form-group">
                            <div class="col-lg-offset-2 col-lg-10">
                              <input type="submit" name="update_password" value="update_password" class="btn btn-primary">
                            </div>
                          </div>
                        </form>
                      </div>
